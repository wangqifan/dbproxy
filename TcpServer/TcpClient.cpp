...
void TcpClient::newConnetion(int sockfd)
{
  ...
  conn->setMessageCallBack(std::bind(&TcpClient::onMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void TcpClient::onMessage(const TcpConnectionPtr& conn, Buffer* interBuffer, ssize_t len) {
  std::string str = interBuffer-> retrieveAsString(len) ;
  front_connector->send(str);
  if(m_messageCallBack != NULL) {
    m_messageCallBack(conn, interBuffer, 0);
  }
}
